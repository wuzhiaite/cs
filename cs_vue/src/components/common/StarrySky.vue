...
export default {
  data () {
    return {
      screenHeight: window.screen.availHeight - 90 // 屏幕高度
    }
  },
  watch: {
    'screenHeight': function (val) { // 监听屏幕高度变化
      var oIframe = document.getElementById('starrySky')
      oIframe.style.height = Number(val) - 90 + 'px' // '120'是页面布局调整，可去除
    }
  },
  mounted () {
    var _this = this
    window.onresize = function () { // 定义窗口大小变更通知事件
      _this.screenHeight = window.screen.availHeight // 窗口高度
    }
